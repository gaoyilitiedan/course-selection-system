<template>
  <div class="messages-page">
    <el-card>
      <template #header><span>师生留言</span></template>
      ...
      <el-table :data="messages" v-loading="loading" border stripe>
        ...
        <el-table-column label="发送方" width="100">
          <template #default="{ row }">{{ row.fromUserType === 'STUDENT' ? '我' : '教师' }}</template>
        </el-table-column>
...
const form = reactive({ courseId: null, teacherId: null, content: '' })
...
async function fetchMessages() {
  loading.value = true
  try { messages.value = (await studentApi.getMessages()) || [] } finally { loading.value = false }
}

async function sendMsg() {
  if (!form.courseId || !form.teacherId || !form.content) { ElMessage.warning('请填写完整信息'); return }
  sending.value = true
  try {
    await studentApi.sendMessage({ courseId: form.courseId, teacherId: form.teacherId, content: form.content })
