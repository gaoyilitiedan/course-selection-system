<template>
  <div class="crud-page">
    <el-card>
      <template #header><span>本学院成绩管理</span></template>
      <el-form :model="form" inline class="grade-form">
    ...
    <el-card class="mt-16">
      ...
      <el-table :data="grades" v-loading="loading" border stripe>
        ...
        <el-table-column prop="status" label="状态" width="100">
          <template #default="{ row }"><el-tag :type="STATUS_CONFIG[row.status]?.type">{{ STATUS_CONFIG[row.status]?.label }}</el-tag></template>
        </el-table-column>
...
async function fetchGrades() { loading.value = true; try { grades.value = (await deptApi.getGrades()) || [] } finally { loading.value = false } }
async function handleSubmit() {
  if (!form.enrollmentId || form.score == null) { ElMessage.warning('请填写选课记录ID和成绩'); return }
  submitting.value = true
  try { await deptApi.submitGrade({ enrollmentId: form.enrollmentId, score: form.score }); ElMessage.success('成绩录入成功'); form.enrollmentId = null; form.score = null; fetchGrades() } finally { submitting.value = false }
}
onMounted(fetchGrades)
</script>

<style scoped>
.grade-form { margin-bottom: 0; }
